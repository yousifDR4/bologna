...
import { get_prof, get_progs } from "../../../../store/getandset";
...
const ExamComitte=()=>{
    ...
    const [committes,setCommittes]=useState(initcommittes);
    ...
    return(
        ...
        <AppBar position="static" sx={{borderTopLeftRadius:"10px",borderTopRightRadius:"10px",bgcolor:"transparent",boxShadow:"none",}}>
          <Toolbar sx={{paddingLeft:"0!important"}}>
            ...
            <InputLabel id="semester" sx={{color:"var(--styling1) !important"}}>Semester</InputLabel>
            <Select
          id="semester"
          label="Semester"
          labelId="semester"
    ...
    variant="outlined"
        ...
        { 
         committes.filter((committe)=> committe.semester===selectedSemester).length < 1 ? <Typography variant="h6" sx={{fontFamily:"Graphik",color:"var(--styling1)",width:"100%",textAlign:"center"}}>No Committees were found!</Typography>:
            committes.filter((committe)=>committe.semester===selectedSemester).map((committe)=>{
                return(
     <ListItem key={committe.id} sx={{width: '19%',minWidth:"250px",boxShadow:"rgba(0, 0, 0, 0.24) 0px 3px 8px",display:"flex",flexDirection:"column",gap:"0.5rem",bgcolor:"#fff"}}>
                <GroupRoundedIcon sx={{width:"3rem",height:"3rem",color:"var(--styling1)",background:"var(--backGround)",borderRadius:"50%",padding:"0.5rem"}}/>
                <Accordion sx={{boxShadow:"none",border:"1px solid #d1d7dc",fontFamily:"GraphikLight"}}>
        <AccordionSummary
          expandIcon={<ExpandMoreIcon />}
          aria-controls="panel1-content"
          id="panel1-header"
        >
          {committe.program.name.toLocaleUpperCase()}
        </AccordionSummary>
        <AccordionDetails>    
     <List sx={{  display:"flex",flexWrap:"wrap"}}>
      <ListItem sx={{padding:"0"}}>
        <StyledListItemText primary="Committe Establishemnet Order Number" secondary={committe.establishNo} />
      </ListItem >
      <ListItem sx={{padding:"0"}}>
        <StyledListItemText primary="Committe Establishment Order Date" secondary={committe.establishDate} />
      </ListItem>
      <ListItem sx={{padding:"0"}}>
        <StyledListItemText primary="Checking Commite Establishment Order No." secondary={committe.checkENO} />
      </ListItem>
      <ListItem sx={{padding:"0"}}>
        <StyledListItemText primary="Checking Committe Establishment Order Date" secondary={committe.checkEDate} />
      </ListItem>
      <ListItem sx={{padding:"0"}}>
        <StyledListItemText primary="Notes" secondary={committe.notes.length > 0 ? committe.notes : "None" } />
      </ListItem>
    </List>  
    </AccordionDetails>
    </Accordion>
    <Box sx={{display:"flex",gap:"0.5rem",width:"100%",marginTop:"0.8rem"}}>
    <AddCommitte programs={programs} initialValues={committe} edit={true}/>
        ...
        <CommitteMembers edit={true} initialValues={committe} professors={professors} program={programs.filter((prog)=>prog.id===committe["program"]["id"])[0]}/>
            </ListItem>
                )
            })}
        </List>
