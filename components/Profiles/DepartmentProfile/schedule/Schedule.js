...
const Schedule=()=>{
    ...
    const [selectedProgramObject,setSelectedProgramObejct]=useState(programs.filter((p)=>p.id===selectedProgram)[0]);
    ...
    let disableButton=selectedDivision !== "" && selectedLevel !== "" && selectedProgram !== "" && selectedSpeciality !== "";
    return(
    ...
    variant="outlined"
        ...
        <Box sx={{overflow:"auto",width:"100%",maxWidth:"100%",boxSizing:"border-box"}}>
        <ScheduleTable modules={modulesSch} timeStart="8:30" timeEnd="14:30" classRooms={classrooms} modulesList={modules}/>
        </Box>
        </Box>
    );
}
