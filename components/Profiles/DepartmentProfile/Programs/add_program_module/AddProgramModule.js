...
import { Add, Book, Save, Upload } from "@mui/icons-material";
...
const AddProgramModule = () => {
  ...
  const [completeForm, setCompleteForm] = useState({
    ...
    MAST: false,
    MLS: false,
  }); //used in progress bar and classes style
  const [rightContainer, setRightContainer] = useState(
    <ModuleInfo
      title="MInfo"
      setFormIsValid={setCompleteForm}
      setForm={setForm}
      setm={setModules}
      form={form}
    />
  ...
  const updateProgress = () => {
    //update Progress bar
    const trueCount = Object.values(completeForm).filter(
      (value) => value
    ).length;
  ...
  const submithandler = async () => {
    ...
    try {
      const filteredObject = Object.entries(form).reduce(
        (acc, [key, value]) => {
          if (value !== "") {
            acc[key] = value;
          }
      ...
      dispatch(
        messageActions.setMessage({
          messageContent: "The Module was added succesfully!",
          severity: "success",
        })
      );
...
console.log(uploading);
  ...
  return (
    <ErrorRoundedIcon
      sx={{ color: "#B8BFC6", verticalAlign: "middle", marginLeft: "0.2rem" }}
    />
  );
};
